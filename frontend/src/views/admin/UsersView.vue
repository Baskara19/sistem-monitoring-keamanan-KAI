...
const fetchLocations = async () => {
  ...
  try {
    const response = await axios.get("http://127.0.0.1:8000/api/admin/locations", getAuthHeaders());
...
const saveLocation = async () => {
  ...
  try {
    if (locationEditMode.value && selectedLocation.value) {
      await axios.put(
        `http://127.0.0.1:8000/api/admin/locations/${selectedLocation.value.id}`,
        {
...
const toggleLocationStatus = async (location) => {
  ...
  try {
    await axios.put(
      `http://127.0.0.1:8000/api/admin/locations/${location.id}`,
      {
        name: location.name,
        status: newStatus,
      },
...
const submitForm = async () => {
  ...
  try {
    ...
    if (modalMode.value === "create") {
      ...
      await axios.post("http://127.0.0.1:8000/api/admin/users", createData, getAuthHeaders());
    }
...
const deleteUser = async (id) => {
  ...
  try {
    await axios.put(
      `http://127.0.0.1:8000/api/admin/users/${id}/archive`,
      {
        status: "nonaktif",
      },
      getAuthHeaders(),
