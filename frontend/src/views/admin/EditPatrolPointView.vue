...
const patrolPointId = route.params.id;
...
const submitForm = async () => {
  ...
  if (!form.value.name.trim()) {
    error.value = "Nama titik patroli wajib diisi.";
    return;
  }
  ...
  try {
    ...
    await Swal.fire({
      icon: "success",
      title: "Berhasil!",
      text: "Titik patroli berhasil diperbarui.",
      confirmButtonText: "OK",
    });
    ...
    if (err.response?.status === 422) {
      ...
      error.value =
        err.response?.data?.message || "Terjadi kesalahan saat memperbarui titik patroli.";
    }
  }
};
...
const loadPatrolPoint = async () => {
  try {
    ...
    const point = response.data.patrol_point;

    if (!point) {
      error.value = "Titik patroli tidak ditemukan";
      return;
    }
    ...
    form.value = {
      ...
      radius_meters: point.radius_meters ?? 50,
    ...
    <main class="main-content">
      ...
      <header class="topbar">
        <div class="page-heading">
          <h1>Edit Titik Patroli</h1>

          <p>Perbarui informasi titik patroli yang sudah terdaftar.</p>
        </div>
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="form-footer">
              ...
              <button type="submit" class="btn-primary" :disabled="loading">
                {{ loading ? "Menyimpan..." : "Update Titik Patroli" }}
              </button>
            </div>
          </form>
        </div>
